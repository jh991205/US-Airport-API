...
for(i in 1 : nrow(data)) {
  points_reachable_from_3_75<- points_reachable_from(c(lonlat_usa[,1][i],lonlat_usa[,2][i]), x = 75, direct_from_3)
  prop = nrow(points_reachable_from_3_75)/nrow(lonlat_usa)
  data[,3][i] = prop
  acc  = acc + prop
}
saveRDS(data, file = "direct_from_3_75.rds")
