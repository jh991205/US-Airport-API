...
for(i in 1 : nrow(data)) {
  points_reachable_from_1_75<- points_reachable_from(c(lonlat_usa[,1][i],lonlat_usa[,2][i]), x = 75, direct_from_1)
  prop = nrow(points_reachable_from_1_75)/nrow(lonlat_usa)
  data[,3][i] = prop
  acc  = acc + prop
}
saveRDS(data, file = "direct_from_1_75.rds")
