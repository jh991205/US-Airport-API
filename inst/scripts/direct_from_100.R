...
for(i in 1 : nrow(data)) {
  points_reachable_from_100<- points_reachable_from(c(lonlat_usa[,1][i],lonlat_usa[,2][i]), x = 100, direct_from)
  prop = nrow(points_reachable_from_100)/nrow(lonlat_usa)
  data[,3][i] = prop
  acc  = acc + prop
}
saveRDS(data, file = "original_data_100.rds")
